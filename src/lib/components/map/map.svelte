...
let selectedType = "all";
  ...
  //   // Función para seleccionar el icono correcto según el tipo
  //   const getIconForLocation = (type) => {

  //     const iconSize = type === "parking" ? [50, 30] : [25, 25]
  //     return L.icon({
  ...
  //       // Añadir marcadores para cada ubicación en $locations con el icono correspondiente
  //       $locations.forEach((lugar) => {
  //         const marker = L.marker([lugar.lat, lugar.lng], {
  //           icon: getIconForLocation(lugar.type),
  //         }).addTo(map);
  //         marker.bindPopup(
  //           `<b>${lugar.type}: ${lugar.id}</b><br>${lugar.name}`
  //         );
  //       });
  //     });
